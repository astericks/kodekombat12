...
var zl,zb;
	...
	no_of_moves=reverse_trace.length;
	col=reverse_trace[0].length;
	
	for(z=0;z <no_of_moves;z++)
	{
    	for(zb=0;zb<24;zb++)
    	{
    	    trace[z][zb]=reverse_trace[z][zb];
...

{
	...
	if(!((i2==no_of_moves) && (trace[i2-1][22]==-1)))
	{
                   if (dxP != 0 || dyP != 0)
	...
	if(!((i2==no_of_moves) && (trace[i2-1][23]==-1)))
	{
	 ...
	 //Player1's Ghost 'D':
        ...
        else sghost(trace[i2 - 1][7], trace[i2 - 1][6], 1);
...

{
    ...
    //call doMove every 500 milli seconds
