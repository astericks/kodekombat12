...
int ghost_inactive1,ghost_inactive2;
...
int main(int argc, char *argv[])
{
	...
	else if(compile_status==3)
	{
		f1=-2;
		f2=-2;
		write_trace(ghost_p1,pacman1_x,pacman1_y,ghost_p2,pacman2_x,pacman2_y);
		printf("COMPILE ERROR:Both bots lose!\n");
		exit(1);
	}

	

	//sprintf(exec1,"timeout 5s ./%s/pacman < ./%s/state.txt > ./%s/move.txt",bot1,bot1,bot1);
	//sprintf(exec2,"timeout 5s ./%s/pacman < ./%s/state.txt > ./%s/move.txt",bot2,bot2,bot2);
sprintf(exec1," ./%s/pacman.exe < ./%s/state.txt > ./%s/move.txt",bot1,bot1,bot1);
      sprintf(exec2," ./%s/pacman.exe < ./%s/state.txt > ./%s/move.txt",bot2,bot2,bot2);
	...
	for(i=0;i<4;i++)
	{
		...
		init_ghost_p1[i][1]=ghost_p1[i][1];
		init_ghost_p2[i][1]=ghost_p2[i][1];
	}
	...
	for(i=0;i<1000;i++)
	{
		...
		if(exec_status1!=0||exec_status2!=0)
		{
			...
			else if(exec_status2==TIME_LIMIT_EXCEEDED||exec_status2==SEGMENTATION_FAULT||exec_status2==ARITHMETIC_EXCEPTION||
																exec_status2==RUNTIME_ERROR)
			{
		...
		if(ghost_inactive1==10)
		{
			f1=0;ghost_inactive1=0;
		}
		if(f2==1)
		{
			ghost_inactive2++;
		}
		if(ghost_inactive2==10)
		{
			f2=0;ghost_inactive2=0;
		}
	}
...
{
	char dir1[20],dir2[20];
	sprintf(dir1,"rm %s",bot1);
	sprintf(dir2,"rm %s",bot2);
	system(dir1);
	system(dir2);
}
...
int write_state_file()
{
	...
	fprintf(fp1,"%d %d",ghost_inactive1,ghost_inactive2);
	fprintf(fp2,"%d %d",ghost_inactive2,ghost_inactive1);
...
void write_trace(int ghost_p1[4][2], int pacman1_x,int pacman1_y,int ghost_p2[4][2],int pacman2_x,int pacman2_y)
{
	FILE *fp;
	int i;
	fp=fopen("../viewer/trace.txt","a");
	if(fp==NULL)
		printf("Cannot open trace\n");
	for(i=0;i<4;i++)
	{
...
int read_state(char map[20])
{
	...
	sprintf(cmd,"cp ../maps/%s ../viewer/map.txt",map);
	system(cmd);
	fp=fopen("../viewer/map.txt", "r");
...
void get_current_positions(int ghost_p1[4][2], int *pacman1_x,int *pacman1_y,int ghost_p2[4][2], int *pacman2_x,int *pacman2_y)
{
	...
	for(i=0;i<map_row;i++)
	{
		...
		{	
			...
			if(ch2=='P'){
				*pacman2_x=i;
				*pacman2_y=j;		//printf("%d %din get pos\n",*pacman2_x,*pacman2_y);	
			}
		}
	}
...
void change_state(int ghost_p1[4][2],int pacman1_x,int pacman1_y,int ghost_p2[4][2],int pacman2_x,int pacman2_y)
{
	...
	if(flag1==1&&flag2==1)
	{
		printf("Both pacmans lose! output error");
		f1=OUTPUT_ERROR;
		f2=OUTPUT_ERROR;
		write_trace(ghost_p1,pacman1_x,pacman1_y,ghost_p2,pacman2_x,pacman2_y);
		exit(1);
	...
	/*to check if any of the ghosts controlled by player 1 overlap*/
	while(move_flag1==1)
	{
	for(i=0;i<4;i++)
		{
	...
	/*to check if any of the ghosts controlled by player 2 overlap*/
	while(move_flag2==1)
	{
	for(i=0;i<4;i++)
		{
	...
	/*to check if any of the ghosts controlled by player 1 or 2 overlap*/
	while(move_flag3==1)
	{
	for(i=0;i<4;i++)
		{
	...
	else if(map_state1[new_pacman1_x][new_pacman1_y]=='E') {score1+=10; f1=1;ghost_inactive1=0;}


	if(map_state2[new_pacman2_x][new_pacman2_y]=='e') score2++;
	else if(map_state2[new_pacman2_x][new_pacman2_y]=='E') {score2+=10; f2=1;ghost_inactive2=0;}
...
void dead_ghost(int *r,int *c)
{
	int i,j;
	int rand_r,rand_c,temp_rand;
	int f=0;
	rand_r=rand()%(map_row);
	rand_c=rand()%(map_col);
	printf("%d rand\n",rand_r);
	...
	while(1)
	{
		if(map_state1[rand_r][rand_c]=='.'||map_state1[rand_r][rand_c]=='e'||map_state1[rand_r][rand_c]=='E')
		{
			*r=rand_r;*c=rand_c;return;
		}
		rand_r=(rand_r+1)%map_row;
		if(rand_r==temp_rand)
			rand_c=(rand_c+1)%map_col;
